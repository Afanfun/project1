  ...
  export default {
    ...
    methods: {
      ...
      handler:throttle(async function(type,disNum,cart){
        ...
        switch(type){
          ...
          case "add":
            ...
            break;
          case "minus":
            // 判断产品的个数大于1,才可以传递给服务器-1 , 否则传0
            disNum = cart.skuNum > 1 ? -1 : 0;
            break;
          case "change":
            // 如果用户输入进来最终量是非法的(带汉字|负数),带给服务器0(原封不动)
            if(isNaN(disNum)||disNum<1){
              disNum = 0;
            }else{
              // 属于正常情况(小数->取整),带给服务器变化的量:用户输入的量 - 产品的起始个数
      ...
      async updateChecked(cart,event){
        // console.log(event.target.checked);  // 返回的是布尔值;带给服务器的参数不是布尔值,而应该是0或1
